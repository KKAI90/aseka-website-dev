...
export default function CompanyInfo() {
  return (
    <section id="company" style={{ padding: "120px 60px", background: "var(--dark)" }}>
      ...
      {/* Info grid */}
      <div className="info-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2px" }}>
        {rows.map((row, i) => (
          <div key={i} style={{
            display: "flex",
            ...
            <div style={{
              flexShrink: 0, width: "200px", padding: "24px 32px",
            ...
            <div style={{
              ...
              {row.href ? (
                <a href={row.href} target={row.target} style={{ color: "var(--gold-light)", textDecoration: "none" }}>
                  {row.value}
                </a>
              ) : row.value}
            </div>
      ...
      <div style={{
        ...
        <p style={{
          ...
          株式会社ASEKAは、日本とベトナムをつなぐ人材支援のプロフェッショナルとして、企業と求職者双方の夢の実現に貢献してまいります。
        </p>
      </div>
