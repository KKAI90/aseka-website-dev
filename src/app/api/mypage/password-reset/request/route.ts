...
export async function POST(req: NextRequest) {
  try {
    ...
    const token = signPasswordResetToken(cand.id);
    ...
    const link = `${origin}/mypage/reset-password?token=${encodeURIComponent(token)}`;

    if (!mailerConfigured()) {
      console.error("GMAIL_USER / GMAIL_APP_PASSWORD not set — cannot send password reset email. Link:", link);
    ...
    await transporter.sendMail({
      ...
      to: cand.email,
      subject: "【ASEKA】パスワード再設定 / Đặt lại mật khẩu",
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px;background:#fff;border:1px solid #e5e7eb;">
          ...
          <p style="font-size:14px;color:#333;line-height:1.7;">
            下記のボタンから新しいパスワードを設定してください（30分間有効）。<br/>
            <span style="font-size:12px;color:#6b7280;">Nhấn nút bên dưới để đặt mật khẩu mới (link có hiệu lực trong 30 phút).</span>
          </p>
          <div style="text-align:center;margin:28px 0;">
            <a href="${link}" style="display:inline-block;padding:12px 28px;background:#C8002A;color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:14px;">
              パスワードを再設定する / Đặt lại mật khẩu
            </a>
          </div>
          <p style="font-size:11px;color:#9BA0AC;">
            このメールに心当たりがない場合は無視してください。パスワードは変更されません。<br/>
            Nếu bạn không yêu cầu email này, vui lòng bỏ qua. Mật khẩu của bạn sẽ không thay đổi.
          </p>
        </div>
      `,
    });
