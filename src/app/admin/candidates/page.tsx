          ...
          {selected&&(
            <div style={{background:"#fff",...B,borderRadius:"10px",height:"fit-content",position:"sticky",top:"16px",overflow:"hidden"}}>
              ...
              <div style={{padding:"12px 16px",borderTop:"0.5px solid rgba(11,31,58,0.08)"}}>
                ...
                {/* This one button covers both "view" and "export": the popup it opens
                   already renders in the exact reference-rirekisho layout (photo, header
                   table, 学歴/職歴/免許・資格, 添付書類 pages) and its own 🖨 印刷・PDF保存
                   button hands PDF export to the browser's print pipeline — the same
                   pipeline verified (via Playwright's page.pdf()) to paginate correctly and
                   render Japanese text without corruption. The old CV書き出し button here
                   generated a completely different, generic HTML file (no photo, no ID
                   images, no 健康状態/扶養家族 sections) that never matched this format;
                   removed rather than fixed in place, since duplicating the popup's layout
                   into a second export path would just be two places to keep in sync. */}
                ...
                <div style={{display:"flex",gap:"6px"}}>
                  ...
                  <button onClick={()=>deleteCandidate(selected.id)} style={{flex:selected.email?"none":1,padding:"7px 10px",borderRadius:"7px",fontSize:"11px",fontWeight:600,background:"#FCEBEB",color:"#A32D2D",border:"0.5px solid #F09595",cursor:"pointer"}}>{t("common.delete")}</button>
                </div>
              </div>
            </div>
          )}
